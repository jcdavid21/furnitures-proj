<?php 
    ...
    if(isset($_POST["itemId"]) && isset($_POST["itemName"]) && isset($_POST["itemPrice"]) && isset($_POST["itemSrc"]))
    {
        ...
        if($result->num_rows > 0)
        {
            ...
            $query = "INSERT INTO tbl_cart(item_id, acc_id, item_name, item_price, item_src)
            VALUES(?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("iisis", $item_id, $acc_id, $item_name, $item_price, $item_src);
            $stmt->execute();
